...
**Source of truth:** produced by `/analyze-frontend` + `/create-frontend-docs` in the target project.

**Step 1 — read `architecture.organizing_principle` from `frontend-analysis.json`:**
- `"fsd"` → use FSD layer rules below
- anything else → read `## Component Placement Rules` from `reference-component-creation-template.md`

**Step 2a — FSD: classify by component type**

| Condition | Layer | Path pattern |
| ---- | ---- | ---- |
| Generic UI, no domain noun (Button, Icon, Badge, Input) | `shared/ui` | from `architecture.top_level_dirs` |
| Domain entity (UserCard, ProductBadge) | `entities/<name>` | from `architecture.top_level_dirs` |
| User interaction (LoginForm, AddToCart) | `features/<name>` | from `architecture.top_level_dirs` |
| Composite block | `widgets/<name>` | from `architecture.top_level_dirs` |
| Full page | `pages/<name>` | from `architecture.top_level_dirs` |

Exact directory names come from `architecture.top_level_dirs` in `frontend-analysis.json` — not hardcoded.

**Step 2b — Non-FSD: read `## Component Placement Rules` from `reference-component-creation-template.md`**

This section is generated by `create-frontend-docs` from the project's actual structure.
If the section is absent (Gap B not yet filled) → ask user where to place the component.

**In Phase 0.5 plan — always show detection result:**

```
Layer detection:
  organizing_principle: fsd  (frontend-analysis.json)
  top_level_dirs: src/shared/ui, src/entities, src/features, src/widgets
  Component type: primitive → src/shared/ui/button/
```
